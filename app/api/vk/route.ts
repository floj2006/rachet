...
const VK_BOT_LINK = process.env.NEXT_PUBLIC_VK_BOT_URL ?? "https://vk.com/your_group";
...
export async function POST(req: NextRequest) {
  ...
  if (body.type === "message_new") {
    ...
    if (!userId || userId < 0) return plainText("ok"); // ignore group messages

    const keywords = ["оплатил", "оплата", "оплатила", "перевел", "перевела", "заплатил"];
    const isPaid = keywords.some((kw) => text.includes(kw)) || text === "/start";

    if (isPaid) {
      const existing = getPending(userId);
      if (existing) {
        await vkSendMessage(
          userId,
          "⏳ Ваша заявка уже на проверке у администратора. Ожидайте — обычно это занимает до 30 минут."
        );
      } else {
        const userName = await vkGetUserName(userId);
        createPending(userId, userName);
        await vkSendMessage(
          userId,
