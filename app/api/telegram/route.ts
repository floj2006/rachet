...
export async function POST(req: NextRequest) {
  ...
  if (data?.startsWith("confirm_")) {
    ...
    const pending = await getPending(vkUserId);
    ...
    if (chatId && messageId) {
      await editMessageText(chatId, messageId, `✅ Подтверждено\n\n👤 ${pending.userName} (VK ID: ${vkUserId})\n🔑 Токен: ${token}`);
    }
  }
  ...
  if (data?.startsWith("reject_")) {
    ...
    if (pending) {
      await deletePending(vkUserId);
      await vkSendMessage(vkUserId, "❌ К сожалению, оплата не подтверждена. Если вы считаете, что произошла ошибка — напишите нам напрямую.");
    }
